...
function currencyFormatter($number, $precision = 8) {
    ...
    return rtrim(number_format($number, $precision), 0);
}
...
function nameThumb($name) {
    $words  = explode(' ', $name);
    $result = $words[0][0] . $words[1][0];
    return strtoupper($result);
}

function referralBonus($referralUser, $refBonus, $detail) {
    $referralUser->balance += getAmount($refBonus);
    $referralUser->save();

    $transaction               = new Transaction();
    $transaction->user_id      = $referralUser->id;
    $transaction->amount       = getAmount($refBonus);
    $transaction->charge       = 0;
    $transaction->post_balance = getAmount($referralUser->balance);
    ...
    $general = gs();
    notify($referralUser, 'REFERRAL_BONUS', [
        'username'      => $referralUser->username,
        'amount'        => getAmount($refBonus),
        'trx'           => $transaction->trx,
        'site_currency' => $general->cur_text,
        'post_balance'  => showAmount($referralUser->balance),
        'detail'        => $detail,
    ]);
